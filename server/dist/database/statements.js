...
function saveUserAndTracks(user, tracks) {
    ...
    `INSERT OR IGNORE INTO users (spotify_user_id, display_name)
        VALUES (?, ?)`, [user.id, user.display_name], function (err) {
        if (err)
            return console.error(err);
        let userId = this.lastID;
        if (!userId) {
            db.get(`SELECT id FROM users WHERE spotify_user_id = ?`, [user.id], (err, row) => {
...
function insertTracks(userId, tracks) {
    tracks.forEach((track, index) => {
        db.run(`INSERT OR IGNORE INTO top_tracks
            (user_id, spotify_track_id, track_name, artist_name, rank)
            VALUES (?, ?, ?, ?, ?)`, [userId, track.id, track.name, track.artist, index]);
    });
